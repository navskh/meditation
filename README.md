...
## ✍️ 오서링 앱 (본문 불러오기 + 메모 + 발행)

`app/` 폴더는 **로컬에서 실행하는 작은 백엔드 앱**입니다. 개역한글 본문을 불러와 구절에 메모를 달고, 묵상을 정리해 **발행 버튼 한 번으로 저장 → 목록 갱신 → git push** 까지 끝냅니다. (공개 뷰어 `index.html`은 정적이라 이런 작업을 못 하므로 역할을 나눴습니다.)

**개역한글 본문 데이터**: `app/data/bible-krv.json` (66권 전체, 약 4.6MB) — 네트워크 없이 동작합니다.
...
- 브라우저에서 열면 **3분할** 화면: ① 본문(개역한글) · ② 묵상 정리 · ③ 미리보기(인쇄 모습 그대로)
  1. **본문** 칸에 `창세기 19장` / `요한복음 3:16-18` / `시편 23` 처럼 입력하고 **불러오기** (약어 `창 19`, `요 3:16`도 됨)
  2. 절 번호를 누르면 **묵상에 담기**, 본문을 **드래그**하면 그 구절에 **메모**를 달 수 있어요
  3. ② 칸에 전체 묵상을 자유롭게 정리 → 담은 구절 + 메모 + 묵상이 ③에서 하나로 합쳐집니다
- 상단에서 **주제 / 날짜**를 정한 뒤 **📮 발행** → 1~2분 뒤 <https://navskh.github.io/meditation/> 에 반영
