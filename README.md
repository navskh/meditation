...
`app/` 폴더는 **로컬에서 실행하는 작은 백엔드 앱**입니다. 에디터에서 글을 쓰고, **발행 버튼 한 번으로 저장 → 목록 갱신 → git push** 까지 끝냅니다. (공개 뷰어 `index.html`은 정적이라 이런 작업을 못 하므로 역할을 나눴습니다.)

```
✍️ app/editor.html (로컬, 백엔드 있음)  →  쓰기 + 📮 발행
...
- 브라우저에서 열면 **2분할** 화면: 왼쪽 마크다운 에디터 · 오른쪽 실시간 미리보기(인쇄 모습 그대로)
